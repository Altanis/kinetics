...
function calculateApothem(width: number, height: number, centerX: number, centerY: number) {
...
/** CONTROLS */
const showHitbox = document.getElementById('showHitbox') as HTMLInputElement;
const showVectors = document.getElementById('showVectors') as HTMLInputElement;
...
const system = window.system = new System({
    ...
    render: {
        ...
        // gridSize: cellSize,
        hooks: {
            preRender: function(context: CanvasRenderingContext2D) {
            ...
            postRender: function(context: CanvasRenderingContext2D) {
                ...
                /** @ts-ignore */
                const keys = window.keys as Set<number>;
...
system.on("entityCreate", (entity: Entity) => {
    console.log("A new entity has been created.", entity.id);
});

system.on("entityDelete", (entity: Entity) => {
...
function generatePolygon(sides: number, radius: number, rotation: number, centerX = 0, centerY = 0) {
...
/** @ts-ignore */
const arrowVertices: Vector[] = '40 0 40 20 100 20 100 80 40 80 40 100 0 50'
    .split(' ')
    .map((v, i, a) => i % 2 ? new Vector(+a[i - 1], +v) : null)
    .filter(v => v);

/** @ts-ignore */
const chevron: Vector[] = '100 0 75 50 100 100 25 100 0 50 25 0'
    .split(' ')
    .map((v, i, a) => i % 2 ? new Vector(+a[i - 1], +v) : null)
    .filter(v => v);

/** @ts-ignore */
const star: Vector[] = '50 0 63 38 100 38 69 59 82 100 50 75 18 100 31 59 0 38 37 38'
    .split(' ')
    .map((v, i, a) => i % 2 ? new Vector(+a[i - 1], +v) : null)
    .filter(v => v);
console.log(star);
/** @ts-ignore */
const heart: Vector[] = '50 0 100 0 100 50 50 100 0 50 0 0'
    .split(' ')
    .map((v, i, a) => i % 2 ? new Vector(+a[i - 1], +v) : null)
    .filter(v => v);
...
for (let i = 0; i < 0; i++) {
    const ent2 = new Circle({
        form: { vertices: [new Vector(1000 * Math.random(), 1000 * Math.random())] },
        radius: 50,
        ...opts,
    }, system);
...
document.addEventListener("keydown", function ({ key, code }) {
    switch (key) {
        case "ArrowUp": keys.add(Movement.Up); break;
        case "ArrowDown": keys.add(Movement.Down); break;
        case "ArrowLeft": keys.add(Movement.Left); break;
        case "ArrowRight": keys.add(Movement.Right); break;
    }

    switch (code) {
        case "KeyW": keys.add(Movement.Up); break;
        case "KeyS": keys.add(Movement.Down); break;
        case "KeyA": keys.add(Movement.Left); break;
        case "KeyD": keys.add(Movement.Right); break;
    }
});

document.addEventListener("keyup", function ({ key, code }) {
    switch (key) {
        case "ArrowUp": keys.delete(Movement.Up); break;
        case "ArrowDown": keys.delete(Movement.Down); break;
        case "ArrowLeft": keys.delete(Movement.Left); break;
        case "ArrowRight": keys.delete(Movement.Right); break;
    }

    switch (code) {
        case "KeyW": keys.delete(Movement.Up); break;
        case "KeyS": keys.delete(Movement.Down); break;
        case "KeyA": keys.delete(Movement.Left); break;
        case "KeyD": keys.delete(Movement.Right); break;
    }
});
